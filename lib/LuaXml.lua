...
function encode(var,indent,tagValue)
  ...
  if base.type(var)=="table" then
    local tag = var.xml or tagValue or base.type(var)
    ...
    for k,v in base.pairs(var) do -- attributes 
      if base.type(k)=="string" then
        if base.type(v)=="table" and k~="_M" then --  otherwise recursiveness imminent
          tableStr = tableStr..encode(v,indent+1,k)
        elseif k~= "xml" then
          s = s.." "..k.."=\""..raw_encode(base.tostring(v)).."\""
        end
      end
    end
    ...
    elseif #var==1 and base.type(var[1])~="table" and #tableStr==0 then -- single element
      s = s..">"..raw_encode(base.tostring(var[1])).."</"..tag..">\n"
    else
      s = s..">\n"
      for k,v in base.ipairs(var) do -- elements
        if base.type(v)=="string" then
          s = s..indentStr.."  "..raw_encode(v).." \n"
        else
          s = s..encode(v,indent+1)
        end
      end
...
function find(var, tag, attributeKey,attributeValue)
  ...
  -- compare this table:
  if tag~=nil then
    if var.xml==tag and ( attributeValue == nil or var[attributeKey]==attributeValue ) then
      base.setmetatable(var,{__index=xml, __tostring=xml.str})
      return var
    end
  else
